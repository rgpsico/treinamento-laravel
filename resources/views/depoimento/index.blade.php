...
@section('content')
    ...
    <div class="col-6 mb-2 d-flex justify-content-end align-items-end">
        <a href="{{ route('depoimento.create') }}" class="btn btn-success">
            <i class="fas fa-home"></i>
            <span>Adicionar {{ $pageTitle }}</span></a>
    </div>

    @include('depoimento._partials.filter')
                        ...
                        <tbody>
                            @foreach ($model as $value)
                                <tr>
                                    ...
                                    <td>{{ $value->autor ?? '' }}</td>
                                    <td>{{ $value->depoimento ?? '' }}</td>



                                    <td class="d-flex">

                                        <a href="{{ route('depoimento.edit', ['id' => $value->id]) }}"
                                            class=" mr-2 btn btn-info" style="height:40px; padding:10px;">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('depoimento.destroy', ['id' => $value->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
